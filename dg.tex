...
languages: the New Prague School\is{New Prague School} (Sgall, Hajičová, Panevova) developed Dependency Grammar further,
...
Translation projects \citep{Melcuk64a-u,Melcuk81a,Melcuk88a-u,Kahane2003a-u}. Meľčuk left the
Soviet Union towards Canada in the 1970s and now works in Montréal. 


Dependency Grammar is very wide-spread in Germany and among Germanists
...
The following list provides a probably incomplete list of linguists who are/were based in Germany: 
...
for Danish\il{Danish}, Portugese\il{Portugese}, Spanish\il{Spanish}, English\il{English}, Swedish\il{Swedish}, Norwegian\il{Norwegian}, French\il{French}, German\il{German}, Esperanto\il{Esperanto}, Italian\il{Italian}, and
...
%
%% wikipedia: Word grammar is an example of cognitive linguistics, which models language as part of general knowledge and not as a specialised mental faculty.[1] This is in contrast to the nativism of Noam Chomsky and his students.
...
fragments exist:
\begin{itemize}
\item English\il{English}   \citep{MIF85a,SN86a,Hudson89a,ST91a-u}
\item Finnish\il{Finnish}    \citep*{NJL84a-u,JLV86a-u}
\item German\il{German}     \citep{Hellwig86a-u,Hellwig2003a,Hellwig2006a,MS98c-u,GK2001a}
...
%\todostefan{add referecnes for single languages by Karlsson}
%% \item Danish\il{Danish}, 
...
%% \item Swedish\il{Swedish}, 
%% \item Norwegian\il{Norwegian}, 
%% \item Faroese
% Trosterud, Trond (2009). A Constraint Grammar for Faroese. Constraint Grammar Workshop at NODALIDA
% 2009, Odense. NEALT Proceedings Series, Vol 8, pp.1-7. Tartu: Tartu University Library. (ISSN
% 1736-6305) 
%
...
%% \item German\il{German}, 
%% \item Esperanto\il{Esperanto}, 
...
the nominal phrases it combines with to form a complete clause. The verb determines the form of its
dependents, for instance their case.
...
accusative NP (an object). This is depicted by the dependency links between the node representing the verb and the
...
analysis is appropriate was also led within the Dependency Grammar framework
...
An alternative way to depict the dependencies, which is used in Word Grammar\indexwg \citep{Hudson2007a-u}, is provided in Figure~\vref{fig-the-child-reads-the-book-dg-sentence}.
\begin{figure}
...
upwards. Since we have a clear visualization of the dependency relation that represents the nucleus
...
specification of the status as argument or adjunct. So like the grammatical functions in the
previous figures, one can use explicit labels to this extent. German grammars and valence dictionaries often
...
%% And Gerdes \& Kahane 2001 or Debusmann \& Duchier 2001 for toplogical DG.}
far we have seen dependency graphs that had connections to words that were linearized in a
certain order. However, in principle the order of the dependents is not determined by the dependency
...
instance the rule that states that there is always exactly one element in the Vorfeld in
declarative main clauses, the finite verb is in second position.\footnote{\label{fn-Engel-linearization}%
...
} Furthermore, there are linearization rules that concern pragmatic properties, as for instance given before new information, and weak
pronouns have to be placed into the Vorfeld or at the beginning of the Mittelfeld.
...
liberation are used (see for instance Section~\ref{sec-nld-dg} on nonlocal dependencies).\footnote{\label{fn-projective-dg-vs-constitunets}%
  ...
  Dependency Grammars. In Dependency Grammars on just refers to the label of the head (for instance
...
role\is{semantic role} assignment is just a small part of the problem that has to be solved when natural language
...
probably amount into a lexical rule for the personal passive. See \citew[\page
...
rather than on the main verb.\LATER{S: the classical ref for that is Mel'cuk 1988:129-144, who
  gives a bunch of criteria.} 
% Silvain Kahane 11.04.2015
%% for the auxiliary as head Mel'cuk 1988, Hudson 1984, 1990, 2001, etc
%
...
dependency graphs of the sentences in (\mex{1}) given in the Figures~\ref{fig-vl-dg} and~\ref{fig-vi-dg}, we see that only the position of the verb is
different, but the dependency relations are the same, as it should be.\footnote{
  \citet{Eroms2000a} uses the part of speech \emph{Pron} for pronouns like \emph{jeder}
...
\caption{\label{fig-vi-dg}Analysis of \emph{Kennt jeder diesen Mann?} `Does everybody know this man?'}
\end{figure}%
The correct linearization of the verb with respect to its arguments and adjuncts is ensured by
...
was given in Figure~\ref{fig-vl-dg} already.
...
There are several possibilities to analyze nonlocal dependencies in Dependency
Grammar. The easiest is the one we already saw in the previous sections. One just focuses on the dependency relations
and assumes that the order with the verb in second position is just one of the possible
...
\end{figure}%%
This graph differs from most graphs we have seen before in not being projective\is{projectivity}, which means that
...
\end{figure}%
Note that the order of elements in (\mex{0}) is perfectly compatible with statements that refer to
...
An alternative to assuming discontinuous constituents would be to assume additional mechanisms that
...
%% In Kahane, Nasr and Rambow 1998 (ACL) we formalized this orperation and called it lifting.
...
to the node to which it has risen\is{rising} (the top-most V) by a dashed line. Instead of realizing the accusative dependent of \emph{gesehen}
...
Figure~\ref{fig-wen-glaubst-du-dass-dg-rising}, Figure~\ref{fig-wen-glaubst-du-dass-wg} has an
...
otherwise the No-dangling Principle would loose its force since it could trivially be fulfilled \citep[\page 25]{Hudson2000a}.
...
% Klein hat Beispiele wie Schlag. Die würde Tesniere aber vielleicht als
...
coordination,\todostefan{Hudson: this is not constituent structure, since it covers non-constituent coordination} that
...
Such junctions are also assumed for the coordination of verbs. This however is not without problems,
...
\zl
The Tesnièrean analysis in Figure~\vref{fig-dg-adjunct-attachment-wrong} corresponds to (\mex{0}c), while an analysis that
...
even though the syntax does not correspond to the semantic dependencies (see \citew{Chaves2009a} for
...
introductory classes. However, as \citet[\page 285]{SR2012a} pointed out in a rather general
...
%
This analysis is equivalent to analyses in the Minimalist Program\indexmp that assume a NegP\is{category!functional!Neg} and it
...
placed in the \vf. An extensive discussion of the data is provided in \citep{Mueller2003b}. Additional data
have been collected in a research project on multiple frontings and information structure
\citep{Bildhauer2011a}. Any theory that is based on dependencies alone and that does not allow for
...
%  constituency trees (= PS) and DT has been stated by Lecerf 1960, 1961 (in French only). that's a different thing than the equivalence between gramamtical formalisms}
\begin{figure}
\hfill%
\begin{forest}
sn edges
...
respective lexical items and then removes the rules that license the words, one arrives at the following
lexicalized variant of the grammar:
\ea
%\label{bsp-grammatik-psg}
\begin{tabular}[t]{@{}l@{ }l}
{N} & {$\to$ D book}\\          
...
not to allow for flat structures. For instance, \citet[\page 364]{GSag2000a-u} assumed a flat rule for subject
...
Footnore~\ref{fn-flat-dg-rules} on flat rules.
...
The important point to notice here is that the arguments are combined with the head in the order
...
of a phrase structure rule, but note that this machinery can be used to reach even freer orders: by
...
%There are plenty of works about how to relax the projectovity without suppressing it.}
...
There is some dispute going on about the question whether constituency/dependency is
...
sufficient, a claim  that is shared by dependency grammarians (according to \citet{Engel96a}), \citet{Leiss2003a} claims
...
\glt `That Peter comes does not resolve the question whether Klaus plays.'
\z
...
reason is that the dependency relation is reflected one by one in the semantic representation in
...
\caption{\label{fig-dass-peter-kommt-klaert-nicht-ob}The dependency graph of \emph{Dass Peter kommt,
    klärt nicht, ob Klaus spielt.} `That Peter comes does not resolve the question whether Klaus
...
\gll Dass Peter kommt, klärt nicht, ob Klaus kommt.\\
     that Peter comes    resolves not whether Klaus plays\\
\glt `That Peter comes does not resolve the question whether Klaus comes.'
\z
Here the words \emph{dass} and \emph{kommt} appear twice. Without any notion of constituency or
...
meaning that this sentence has: \emph{dass} dominates \emph{kommt} and \emph{kommt} dominates
...
\caption{\label{fig-dass-peter-kommt-klaert-nicht-ob-non-projective}The dependency graph of
  ...
  question whether Klaus comes.' is not unambiguously determined by semantics.}
\end{figure}%
...
constituents back in (see Footnote~\ref{fn-projective-dg-vs-constitunets} on page~\pageref{fn-projective-dg-vs-constitunets}). 
...
As \citet{Oliva2003a} has pointed out, the equivalence of Dependency Grammar and HPSG only holds up as far
as \headvs are concerned.
%\todostefan{S: many DG have several level of representations. You cannot compare HSG wioth only one
%level of MTT. MTT has 7 level of repressentations.} 
...
lexical material and the complete phrase is given in (\mex{1}) (for lambda expressions see Section~\ref{sec-PSG-Semantik}):
\eal
...
So, the conclusion of this section is that the only principled difference between phrase structure grammars and
...
gap \citep[\page 216--217]{ps2}. Similar analyses can be found in Dependency Grammar (\citealp[\page
  ...
  both a head and a dependent simultaneously (\citealp[Chapter 246, §8--11]{Tesniere2015a-u}; \citealp[\page xlvi]{OK2015a}; \citealp[\page
...
Now, the alternative to empty elements are phrasal constructions (see
...
Chapter~\ref{Abschnitt-Phrasal-Lexikalisch} is devoted to the discussion of the question whether
...
% formal foundations: Discontinuität, Semantics Broeker2003a-u
