...
they have the same mother node.\footnote{\emph{Parent node}\is{node!parent} and \emph{child node}\is{node!child} are alternative
  ...
  formalizations of some of the theories discussed later.
}
If a node has two daughters, then we have a binary\is{binary} branching structure\is{branching}\is{branching!binary}.
