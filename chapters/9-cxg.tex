...
Categorial Grammar\indexcg, LFG\footnote{
	See \citew{Alsina96a} and \citew*{ADT2008a,ADT2013a}, however. For more discussion of this point, see
        Sections~\ref{sec-aspect-at-clause-level} and~\ref{Abschnit-phrasal-LI}.%
...
Section~\ref{Abschnit-phrasal-LI} and \citealp{Mueller2006d,Mueller2007d}). This point will not be explained further here. Instead, I will discuss the work of Fillmore and Kay to prepare the reader
...
Examples of more formal works are \citew{KF99a,Kay2002a}, \citew{MR2001a}, and \citew{Goldberg2003a}.
The book by Jean-Pierre Koenig \citeyearpar{Koenig99a} (formerly Berkeley) has been heavily influenced by CxG. Fillmore and Kay's revisions of their earlier work
...
\citep{Steels2003a}. In personal communication (p.\,M.\ 2007) %200 Jahre
he stated that is is a long way to go until robots finally will be able to learn to speak but the
...
As we saw in Section~\ref{Abschnitt-HPSG-Kosntituentenstruktur}, dominance relations in HPSG are modeled like other properties of linguistic objects using feature"=value
...
\multicolumn{2}{|l|}{phon \phonliste{ the man }}\\
\begin{tabular}[t]{|l|}\hline
phon \phonliste{ the } \\\hline
\end{tabular}&%
\begin{tabular}[t]{|l|}\hline
phon \phonliste{ man } \\\hline
\end{tabular}\\
  &\\\hline
\end{tabular}
\z
...
be identified with an element in the valence set of the mother.\footnote{%
  Sets in BCG work differently from those used in HPSG. A discussion of this is deffered to Section~\ref{sec-formal-bcg}.%
...
\ea
The \emph{Subject Construction} with Pollard and Moschier's definition of sets:\\
\onems{
syn$|$cat v\\
val  \ibox{1}\\
...
The second set of problematic data that will be discussed comes from causativization in Turkish \citep{Lewis67a-u}:
\ea
Öl-dür-t-tür-t- \\
`to cause someone to cause someone to kill someone'\\
(kill = cause someone to die)
\z
The causative morpheme \suffix{t} is combined three times with the verb (\emph{tür} is an allomorph of the causative morpheme).
...
If we assume that changes in valence such as passive, causativization and middles\is{Middle Construction} should be described with the same means
...
In CxG, it is often argued that the usage contexts of certain orders differ and we therefore must be dealing with a different construction.
Accordingly, one would have to assume six constructions for sentences with ditransitive verbs in final position
(also see page~\pageref{Regeln-PSG-Abfolge}). An alternative would be to assume that the ordering
variants all have a similar structure and that the information"=structural properties are dependent
on the position of constituents in the respective structure (see \citew{deKuthy2000a} for German and \citew{Bildhauer2008a}
...
In more recent work by Fillmore, Kay, Michaelis and Sag, the Kay \& Fillmore formalization of the
...
\z
The difference between HPSG in the version of \citet{ps2} is that for Sag, Wasow \& Bender, signs do not have
...
\noindent
For comparison, a description is given here with the feature geometry that was assumed in
...
feature geometry in (\ref{feat-geom-swb}) and the restriction in (\ref{meta-konstruktionsstatemnet}).
...
argument concerns the status of the meta statement in (\ref{meta-konstruktionsstatemnet}) and the
...
\eal
\ex 
%Und von wegen, die dort lebenende Menschen hätte keine Bedürfnisse wie Strassen, schulen,
%Krankenhäuser, 
...
In addition to these empirical problems, there is a conceptual problem with (\ref{meta-konstruktionsstatemnet}):
(\ref{meta-konstruktionsstatemnet}) is not part of the formalism of typed feature structures but
...
%% worked out. 
%% }
Therefore,\label{page-sbcg-formalization} grammars which use (\ref{meta-konstruktionsstatemnet}) cannot be described with the normal formalism. The formalization given in
...
Furthermore, the original problem that (\ref{meta-konstruktionsstatemnet}) was designed to solve is
...
By assuming (\ref{meta-konstruktionsstatemnet}), the theoretical inventory has been increased without any explanatory gain.
...
formalism (Ivan Sag, p.\,c.\,2011, See Chapter~\ref{Abschnitt-Generative-Kapazitaet} on computational complexity and generative power).
...
\textsc{gap}.
\ea
Lexical introduction of nonlocal dependencies in SBCG according to \citet[\page 163]{Sag2012a}:\\
\ms{
form & \phonliste{ like }\\
...
\textsc{sem}, and \textsc{cntxt} (see \citealp[\page 98]{Sag2012a} for these features)\footnote{
  ...
  things, an unspecified value would make the respective structures infinitely umbiguous, since
...
vowel. SBCG allows for the selection of phonological information (the feature is called \form here) and one could try to justify this
by pointing out that there are cases of selection in which the form of the selected argument depends
on the form of the head. For example, the indefinite article in English has to be either \emph{a} or
\emph{an} depending on whether the noun it is combined with starts with a vowel or not:
\eal
\ex an institute
\ex a  house
\zl
So \emph{institute} could select for \emph{an}, while \emph{house} selects for \emph{a}. However,
the situation is more complicate since prenominal adjectives influence the choice of determiner:
\eal
\ex a famous institute
\ex an interesting house
\zl
In (\mex{0}) a phrasal nominal constituent is combined with the determiner and this shows that the
phonology of the determiner has to be determined on the phrasal level and cannot be lexically
encoded (unless one is willing to represent information about preceding adjuncts in the head noun
somehow). The general question is whether there are phenomena that require a selection of \formvs by
heads or whether all phonological phenomena are best handled by constraints on phonology on the
phrasal level.
...
reject this change.\footnote{
  In \citew[\page 253]{MuellerGTBuch2} I claimed that SBCG needs an additional feature in comparison
  to other variants of HPSG. As \citet{VanEynde2015a} points out this is not true for more recent variants of HPSG since
 they have the \textsc{synsem} feature, which is now superfluous. (Van Eynde refers to the \textsc{local}
...
The construction in (\mex{0}) can be represented in a similar way to the format used in Chapter~\ref{Kap-Merkmalbeschreibungen}:\is{construction!Determiner Noun}
\ea
\label{AVM-DetNoun}
\ms[DetNoun]{
...
\citew{Reape94a}, \citew{Kathol95a}, \citew{Kathol2000a}, \citew{Crysmann2003c}, and \citew{BS2004a}.\footnote{
	Crysmann, Beaver and Sag deal with coordination phenomena\is{coordination}. For an analysis of coordination in TAG\indextag that also makes use of
...
triangle \citep[Chapter~5]{Langacker87a-u}. Jerome Feldman\aimention{Jerome Feldman} (slides) gives the following formalization:
\is{hypotenuse}
\ea
~\\*[-1.5\baselineskip]
\begin{tabular}[t]{|l@{}l@{~$\leftrightarrow$~}l|}\hline
...
It is then possible to refer to the properties of this object in a schema/construction.

The feature description in (\mex{1}) is provided in the notation from
Chapter~\ref{Kap-Merkmalbeschreibungen}. It is the equivalent to
...
\ea
\emph{Kim} according to \citet[\page 99]{vanTrijp2013a}:\\
\begin{tikzpicture}
\node (sem) {\fbox{
\begin{tabular}[t]{l}
...
\begin{figure}
\includegraphics[width=\textwidth]{Figures/production-parsing-fcg.png}
\caption{\label{fig-matching-merging-trijp}Generation and parsing in FCG \citep[\page 99]{vanTrijp2011a}}
\end{figure}%
...
\caption{\label{fig-as-trijp}Lexical items and phrasal constructions. Figure taken from \citew[\page 122]{vanTrijp2011a}}
\end{figure}%
...
caused motion construction \citet{SvT2011a} suggest a process called \emph{merging}. Merging is seen
as a repair strategy: if an utterance involves an intransitive verb and some other material, the
utterance cannot be processed with matching alone. For example, when processing (\mex{1}), \emph{he sneezed} could be parsed, but \emph{the
  foam} and \emph{off the cappuccino} would be unintegrated.
\ea
...
allowed. There are two problems with this: first, human language is highly ambiguous and in the case
at hand this could result in situations in which there is a reading for an utterance, so that the
repair strategy would never kick in. Consider (\mex{1}):
\ea
\gll Er schlug den Mann tot.\\
     he beat   the man  dead\\
\glt `He beat the man to death.' or `He beat the dead man.'
\z
(\mex{0}) has two readings: the resultative reading in which \emph{tot} `dead' expresses the result of the
beating and another reading in which \emph{tot} is a depictive predicate. So, the problem is that
(\mex{0}) has a reading and therefore repair is not triggered: \emph{schlug} `beat' is used with the transitive
construction and \emph{tot} is an adjunct (see \citealp{Winkler97a}). However, the more likely analysis of (\mex{0}) is the one
...
As Steels and van Trijp note (p.\,320), if merging is allowed to apply freely, utterances like
...
strength of this link would be very low for \emph{dine} and the transitive construction and
...
\end{forest}
\caption{\label{fig-what-did-the-boy-hit}The analysis of \emph{What did the boy hit?} according to \citet{vanTrijp2014a}}
\end{figure}%
...
topic.\footnote{
He uses the following definitions for topic and focus: \emph{Topicality is defined in terms of aboutness: the topic of an utterance
...
current communicative setting.} \citep[\page 15]{vanTrijp2014a}
...
\citep[\page 22]{vanTrijp2014a} and that the evidence from one language does not necessarily mean that the analysis for that language is
...
still has to identify the relations between the phrases. \citep[\page 7]{vanTrijp2014a}
\end{quote}
...
\glt `Therefore that Peter comes resolves whether Klaus plays.'
\ex[*]{
...
he states that:
\begin{quote}
whereas the filler-gap analysis cannot explain why \emph{do}-support does not occur
...
approach. \citep[\page 22]{vanTrijp2014a}
\end{quote}
...
suggested,\footnote{
  ...
  implemented in the Babel System \citep{Babel}. See \citep{Mueller99h} on restricting
  ...
  linearizeation"=based approaches were replaced by more traditional variants that allow for
...
in Section~\ref{Asbchnitt-GB-Formalisierung} and Section~\ref{sec-formalization-minimalism}). Apart from this, a computer implementation of SBCG is
...
The following code shows how (\mex{1}b) is implemented in TRALE:
\begin{verbatim}
...
grammar. In fact, (\mex{0}) is one way to put the meta constraint in (\ref{meta-konstruktionsstatemnet}).
...
nineties could be used to process SBCG. The question whether this is efficient is an engineering
...
\subsubsubsection{Static constraints vs.\ dynamic mappings and signature and grammar vs.\ open-endedness}
...
25 pages working out the alledged differences. As I have shown in Section~\ref{sec-fcg}, HPSG and FCG are
