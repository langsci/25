...
suggested refocussing the theoretical developments towards models that have to make minimal
...
Minimalist work and the reader is refered to these journals to get an idea about who is active in
this framework. 
...
phase and is sent to the interfaces before the whole sentence is completed.\footnote{
  ...
  are send off to the interface then the top-most phrase in a derivation would never be sent to the interfaces, since it does not depend on any head.
}
...
In the trees to the left and in the middle of Figure~\vref{fig-ditranisitives-options} the c-command
...
[\littlevbar
 ...
 [VP
   ...
   [\vbar
    ...
    [\textit{Benjamin}]]]]
\end{forest}
\caption{\label{fig-ditranisitives-options}Three possible analyses of ditransitives}
\end{figure}%
...
The structure in Figure~\ref{fig-ditranisitives-little-v} is derived by assuming that the verb \emph{show} starts out
  ...
  [\littlevbar
   ...
   [VP
     ...
     [\vbar
      ...
      [\textit{Benjamin}]]]]]
\end{forest}
\caption{\label{fig-ditranisitives-little-v}Analysis of ditransitives involving movement to \littlev}
\end{figure}%
...
Figures~\ref{fig-tranisitives-little-v} and~\ref{fig-intranisitives-little-v} show the analysis of
...
[\vP
  ...
  [\littlevbar~{[\st{\textit{u}D}]}
   ...
   [VP
      ...
      [Theme]]]]]
\end{forest}
\caption{\label{fig-tranisitives-little-v}Analysis of strictly transitives involving \littlev}
\end{figure}%
...
[\vP
  ...
  [\littlevbar~{[\st{\textit{u}D}]}
   ...
   [ \textit{laugh} {[V]} ]]]
\end{forest}
\caption{\label{fig-intranisitives-little-v}Analysis of intransitives involving \littlev}
\end{figure}%
%
...
I and the other cases by the verb (see Section~\ref{sec-case-asignment}). The assignment of
...
of T in order to assign case, since case assignment is done via Agree. Hence the pronominal argument
of \emph{schlagen} in principle could stay in its object position and nevertheless get nominative
from T. This would solve the problem of the GB analysis that was pointed out by \citet[Section~4.4.3]{Lenerz77}. See
page~\pageref{ex-passive-German-no-movment} for Lenerz' examples and discussion of the
...
\citet{Richards2010a} for recent approaches with derivations which are compared in terms of economy). Nevertheless, there are other aspects of 
...
\citet[\page 297]{Rizzi97a-u} suggests the structure in Figure~\vref{Abbildung-Rizzi} (see Grewendorf \citeyear[\page 85, 240]{Grewendorf2002a}; \citeyear{Grewendorf2009a}, too).
\begin{figure}
\centering
...
information structure and semantics in the category symbols.\footnote{
  ...
  for each semantic role. \Eg agent Reciprocal, Benefactive phrase, Instrumental, Causative,
...
which assumes these kind of structures would have to postulate a Universal Grammar with
the information that features can only be checked in (certain) specifier positions (see Chapters~\ref{chap-innateness} 
and~\ref{chap-acquisition} for more on Universal Grammar and language acquisition).
...
in \citeyear{Chomsky2013a}. The solution to the problem is to assume that all combinations of
...
%% }
%% \glt `The leagal duty for self commitment therefore is dropped for single enterprices as soon and so long as they have reached a poportion of women of 30 percent among the inside directors and the outside directors.'                     
%% \zl 
...
make it possible to label $\gamma$. Chomsky mentions this problem in footnote~40, but does not provide a solution.
}
...
\begin{comment}
% Man muss einfach zulassen, dass about einen Satz nimmt, und dann an der Oberfläche einen Filter
% drübergehen lassen.

\subsection{Bewegungsparadoxien und die Kopientheorie der Bewegung}

\mbox{}\citet[\page 12]{Chomsky2007a} geht davon aus, dass bei internem Merge eine Kopie des intern
gemergeten Elements erhalten bleibt. Wenn \zb die Konstituente $\alpha$ intern gemerget wird, so ist
$\alpha$ zweimal im Baum enthalten: einmal an der Stelle, an der es ursprünglich stand, und ein
zweites Mal im komplexen Element der obersten zweielementigen Menge. Es gibt also keine Spur in der
Analyse von (\mex{1}) sondern einfach eine Kopie der gesamten Konstituente:
\eal\settowidth\jamwidth{(Löschung aller Originale bzw.\ Zwischenkopien)}
\ex Bagles, I like bagles. \jambox{(Internes und Externes Merge)}
\ex Bagles, I like.        \jambox{(Löschung aller Originale bzw.\ Zwischenkopien)}
\zl
Nach Chomsky werden dann an der Schnittstelle zur Aussprache alle identischen Konstituenten bis auf
die letzte gelöscht. 

Eine solche Theorie der Fernabhängigkeiten hat Probleme mit den Bewegungstheorien, die schon seit
einiger Zeit in der Literatur diskutiert werden. \citet[Chapter~2]{Bresnan2001a} diskutiert einige
Fälle ausführlich und ich möchte sie nur kurz wiederholen. Verben wie \emph{talk} verlangen ein
Präpositionalobjekt:
\ea
We talked about that problem for days.
\z
Im Englischen ist es möglich, Präpositionen ohne ihr Objekt zurückzulassen (\emph{Preposition
  Stranding}\is{preposition stranding}:
\ea
{}[That problem]$_i$ we talked about \_$_i$ for days.
\z
Das Paradox entsteht nun, wenn man sich einen Satz wie (\mex{1}) ansieht:
\ea
\label{Beispiel-S-about}
{}[\sub{S} That he was SICK]$_i$ we talked about \_$_i$ for DAYS.
\z
Dieser Satz kann nicht aus (\mex{1}a) entstanden sein, denn ein Satz ist an der entsprechenden
Stelle nicht zulässig. Nur ein NP"=Komplement wie in (\mex{1}b) kann mit \emph{about} kombiniert werden.
\eal
\ex[*]{
We talked about [\sub{S} that he was sick] for days.
}
\ex[]{
We talked about [the fact that he was sick] for days.
}
\zl
Das heißt, wenn man annimmt, dass (\mex{-1}) die Struktur in (\mex{1}) hat, wobei nur die erste
Kopie des Satzes ausgesprochen wird, dann bekommt man ein Problem bei der Analyse der PP, denn die
ist lokal nicht wohlgeformt.
\ea
{}[\sub{S} That he was sick] we talked about [\sub{S} that he was sick] for days.
\z
Gereon Müller\aimention{Gereon Müller} (p.\,M.\ 2009) hat mich darauf hingewiesen, dass solche
scheinbaren Paradoxa in der GB"=Theorie kein Problem darstellen, da ja lokale Inkonsistenzen sogar
eine Triebkraft für Bewegung sind. Man vergleiche die im Abschnitt~\ref{Abschnitt-GB-Passiv}
vorgestellte Analyse des Passivs, in der das Objekt umgestellt werden muss, weil es sonst keinen
Kasus bekommen würde. Man müsste also einen Filter spezifizieren, der grammatische Strukturen
ausschließt, in denen eine Präposition mit einem Satz kombiniert wurde. Die hier diskutierten
Beispiele sind aber geringfügig anders als die Passiv"=Beispiele: Das Problem liegt darin, dass man
den Filter nicht lokal anwenden kann. Würde man (\mex{1}) lokal ausschließen, dann könnte man
komplexere Strukturen, in denen dieser Ausdruck vorkommt nicht ableiten.
\ea[*]{
about that he was thick
}
\z
Man muss (\mex{0}) also lokal zulassen und kann erst, wenn man eine Struktur wie
(\ref{Beispiel-S-about}) aufgebaut hat, den Filter anwenden. Das hat aber zur Konsequenz, dass man
nicht erklären kann, warum (\mex{0}) als einzelne Äußerung (\zb auf die Frage \emph{What did you
  talk about?}) nicht möglich ist. Der einzige Weg (\mex{0}) auszuschließen bestünde darin
anzunehmen, dass PPen, wenn sie wie (\mex{1}a) in Isolation vorkommen, aus ganzen Sätzen wie
(\mex{1}b) durch Löschung von Material entstanden sind:
\eal
\ex about the weather
\ex We talked about the weather.
\zl
Man könnte dann annehmen, dass (\mex{-1}) ausgeschlossen ist, weil ein auf Satzebene operierender
Filter
\end{comment} 
...
Such an approach is problematic for sentences with mono-valent verbs: according to Chomsky's
proposal, subjects of mono-valent verbs would not be specifiers but complements.\footnote{
...
not allow for analyses in which two lexical verbs are directly coordinated as in \pmex{1}:\footnote{\label{fn-Chomsky-on-Specifiers}%
  ...
  remarks will be relevant even under current Chomskian assumptions.
}
...
\eal
\ex[]{\label{ex-tried-pursade-and-convince-him}
...
\zl
The second example is ungrammatical if \emph{him} is not stressed. In contrast, \pref{ex-tried-pursade-and-convince-him} is
well-formed even with unstressed \emph{him}. So, if \pref{ex-tried-pursade-and-convince-him} were an instance of
...
with a determiner or an adjunct. Determiners and mono-valent verbs in controlled infinitives are
...
Pollard and Sag as well as Sag and Ginzburg assume flat structures for English. Since one of the daughters
...
Chapters~\ref{Kapitel-Merkmalstrukturen}, \ref{Kapitel-CG} and~\ref{Kapitel-HPSG} and return here afterwards.
...
the same as the structures created by TAGs. For more on the generative capacity of grammars, see Chapter~\ref{Abschnitt-Generative-Kapazitaet}.
...
\citet[\page 124]{Veenstra98a}: it makes incorrect predictions in the case of mono"=valent verbs\is{verb!mono"=valent|(}. If
...
\zl
To solve this problem, Stabler assumes that mono-valent verbs are combined with a nonovert object\is{empty element}
...
%% bi"=valent unaccusative verbs. For example, the German verb 
%
...
% }
Apart\is{coordination|(} from the problems with mono-valent verbs, this results in the problem that was discussed in
...
mono-valent verbs and the coordination data is unproblematic as well if one assumes that the
...
object position of \emph{like} and then these two copies are related to the fronted element. Koebele
...
extraposition in the HPSG framework, see \citew{Keller95b,Bouma96}, \citew[Chapter~13]{Mueller99a}. 
...
\citet{Chomsky2010a} has compared a Merge"=based analysis of auxiliary movement\is{auxiliary
  movement} to a HPSG analysis and critiqued that the HPSG analysis uses ten schemata rather than
...
auxiliary movement and shows that there are semantic and pragmatic differences between the various contexts. Every
...
elements that are contained in the object that a head combines with. Only features at the top-most node,
...
complexe in languages like German, Dutch\il{Dutch}, Japanese\il{Japanese}, and Korean\il{Korean}. Of course there are analyses of these languages
...
discussion of approaches to language acquisition and the Principles and Parameters model as well as
...
\url{http://www.linguistics.ucla.edu/people/stabler/coding.html}. 31.03.2010.
	...
	and subjects in initial position, for embeded sentences with mono"=valent verbs, for \emph{wh}"=questions with mono"=valent verbs and
	for polar interrogatives with mono"=valent verbs. An element from this set of numerations corresponds to a particular configuration and a
...
\z
In (i), there are two relative clauses with verbs of differing valence, an embedded sentence with a mono"=valent verb and the matrix clause.
...
The discussion of labeling, abandonment of \xbar theory and a comparsion between Stabler's Minimalist Grammars and HPSG from
...
(modulo transformations) almost matches what is assumed in HPSG (featural descriptions for arguments ordered in a valence list according to a 
...
standards: a current example of this is the article \emph{Problems of Projection} by \citet{Chomsky2013a}.\footnote{
  ...
  page~\pageref{quote-Chomsky-Formalisierung}) as well as general scientific methods (Ockham's Razor), the consequence would surely be a return
...
For short: both teachers and students should read the medical record by Sternefeld and Richter. I implore the students not to abandon their studies straight
