...
%\draw (-3.9,-5.8) to[grid with coordinates] (2.7,-0.6);
%\draw[->] (-2.2,-.7) node[anchor=east] {lexicon} -- (-1.3,-.7);
\draw[->] (0,-1) node[anchor=south] (enumeration) {enumeration} --(0,-5)
node[anchor=north, align=center] {LF/CI\\(meaning)};
\node[left= of enumeration.base west, anchor=base] (lexicon) {lexicon};
\draw[->] (lexicon) -- (enumeration);
...
%
\citet[\page 75]{BB2011b-u} assume a BenefactiveP\is{category!functional!Benefactive}
...
or a proliferation of T heads with respective features (see also \citealp{Borsley2006a,BM2020Minimalism}) is the only way to save this architecture. 
...
(just one or two instances of Merge vs. 45, transformations vs. GPSG-style percolation of features).
...
where fillers and gaps do not match and extraction without gaps. \citet{BM2020Minimalism} is another
comparison of Minimalism and HPSG. The authors discuss differences of approach and outlook of the
two frameworks (sociological differences, formalization and exhaustivity), empirical quality of the
