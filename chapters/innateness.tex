...
A single counter example is enough to disprove a universal  claim and in fact, it is possible to
find a language that has verb"=final order but nevertheless has prepositions.\todostefan{Martin:
  Dryer's WALS chapters list dozens of examples of this sort.}
...
element with structural case\is{case!structural} is the subject (\citealp[\page 153]{Mueller2002b};
\citealp[\page 311]{MuellerLehrbuch1}).
...
\zl
Most of the predicates that can be used without subjects can also be used with an expletive subject. An example is given in(\mex{1}):
\ea
...
\citet[\page 61, 124]{Veenstra98a}). Objects of mono-valent verbs are assumed to be empty
...
The\is{subjacency|(}\todostefan{\cite{Kroch89a-u,SWPsubmitted-u}} 
...
} (\citealp[\page 78--79]{Mueller2007c}; \citealp[Section~2.1]{MM2009a}).\todosatz{raggedright
  funktioniert hier leider auch für die Translation. Das muss irgendwie in eine Umgebung.
}
\eal
\ex 
...
\zl
\noindent 
Rizzi's parametrization\is{parameter!subjacency} of the subjacency restriction has been abandoned in many works, and the relevant effects have been
...
(also see \citew[\page 257]{Wunderlich2008a}). This means that there is an independent explanation for the presence of these categories that is not
...
The authors cite Straits Salish\il{Straits Salish} as a language in which there may be no difference between verbs and nouns (also, see \citealp[\page 481]{EL2009b}).
...
claims by Humboldt and \citet[\page 137]{Chomsky2008a} to the contrary.\footnote{
  ...
  Humboldt, Bühler\aimention{Bühler, Karl} and Martinet\aimention{Martinet, Andr{\'e}} claimed that
...
} \citet[\page 131]{PS2010a} discuss Hixkaryana\il{Hixkaryana}, an Amazonian language from the Caribbean language
...
of the adults (\citealp[\page 207]{Samarin84a};
\citealp[\page 209]{Seuren84a}).\todostefan{M: more recent work: Arends, Mufwene, Parkvall} All that can be said considering this lack of evidence is that there are a number
...
This localization was seen as evidence for the innate basis for our linguistic knowledge (also see \citealp[\page 297--314]{Pinker94a}). 
...
\citealp[\page 57]{Bishop2002a}). This is, however, not assumed (also see \citealp*[\page 196]{FHC2005a}). 
...
be shown that the syntax of natural languages in Gold's sense can be learned from texts (also, see \citealp[\page 195--196]{SP2002b}).  
...
Pullum and Scholz note two problems with this PSA: first, they have found hundreds of examples, among them some from
...
The\is{auxiliary movement|(}\il{English|(} fourth PoS argument discussed by Pullum and Scholz comes from Chomsky and pertains to the
...
Examples from CHILDES\is{CHILDES} that disprove the hypothesis that the first auxiliary has to be fronted are given in (\mex{1}):\footnote{ 
  See \citew{LE2001a}. Researchers on language acquisition agree that the frequency of these kind of examples in communication with children is in fact
...
\is{auxiliary movement|)}\il{English|)}
...
\mbox{}\citet{Bod2009a}\is{auxiliary movement|(}\is{statistics|(}\is{Unsupervized Data-Oriented Parsing (U-DOP)|(} 
...
\z
Interestingly, children do produce these kind of incorrect sentences (\citealp[\page 530]{CN87a-u}; \citealp*{ARP2008a}). 
...
for more on this point).\is{auxiliary movement|)}
...
in general.\is{statistics|)}\is{Unsupervized Data-Oriented Parsing (U-DOP)|)}
...
parents all had an academic qualification. They discuss the parent"=child relationship in other cultures, too (see \citew{Ochs82a,OS84a} and \citew[\page 71]{Marcus93a}
