...
\label{Kapitel-Merkmalstrukturen}\label{chap-feature-descriptions}
...
electrical devices, one can use the hierarchy in Figure~\vref{fig-elektr-geraet}.
\begin{figure}
\centering
%% \begin{forest}
%% typehierarchy
%% [\type{electrical applicance}
%%   [\type{printing device}
%%     [\type{printer}
%%       [\type{laser printer}]
%%       [\ldots]]
...
\end{tikzpicture}
\caption{\label{fig-elektr-geraet}Non"=linguistic example of multiple inheritance}
\end{figure}%
The most general type \type{electrical device} is the highest in Figure~\ref{fig-elektr-geraet}. Electrical devices have certain properties, \eg 
...
Figure~\ref{fig-elektr-geraet}. If a type is at the same time the subtype of several superordinate types, then we
...
for \type{printer} $\vee$  \type{photocopier}, one can simply write \type{printing device} if one assumes the type hierarchy in Figure~\vref{fig-elektr-geraet}.%
\is{disjunction|)}
...
\ms{
gender & mas\\
case & nominative $\vee$ dative $\vee$ accusativ~\\
}
\zl
