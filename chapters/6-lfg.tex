...
\Aput*{$\phi$}
\z
The function $\phi$ from the NP"=node to the f"=structure corresponding to the NP is depicted with
an arrow marked $\phi$. 

There is one interesting point to be mentioned with respect to (\mex{0}): while the c-structure is
in \xbar format in general, the I node is missing. Some versions of LFG assume that empty nodes are
not represented in structures but are simply omitted. Hence, VP projects to I$'$ directly without a
combination with I \parencites[Section~4.4]{Dalrymple2001a-u}[\page 84]{Dalrymple2006a}.
...
% Dalrymple2001a-u:269--270 Adjunktion an VP
\ea
\phraserule{VP}{
\rulenode{VP\\* \up~=~\down}
...
is similar to (\mex{1}) for German. A revised version of the analysis of elements in the \mf is
introduced in Section~\ref{Abschnitt-LFG-Umstellung}.
% Bresnan S -> C*
% Dalrymple V -> (V) (NP) PP*
% ZK        S|VP -> NP* (V') (S|VP)
}
...
Note that the verb and all combinations of the verb with arguments are VPs. This is compatible with
the treatment of adjuncts presented in Section~\ref{Abschnitt-LFG-Adjunkte}. Adjuncts can be adjoined to VP nodes and hence
it is explained why adjuncts can appear everywhere in the \mf.
...
other rules. Figure~\ref{fig-revised-v1-Berman} shows the adapted version of the analysis in
Figure~\ref{Abb-Verbstellung-LFG} on page~\pageref{Abb-Verbstellung-LFG}.

\begin{figure}
\centerline{%
\includegraphics{Figures/verschlingt-david-den-apfel-vp-mf-lfg-lsp-crop}
}
\caption{\label{fig-revised-v1-Berman}Revised analysis of a verb-first sentence using Berman's analysis of scrambling}
\end{figure}
