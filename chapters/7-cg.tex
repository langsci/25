...
of this combinaton. The result is given beneath the arrow. Figure~\vref{Abb-CG-als-Baum} shows the tree corresponding to Figure~\ref{abb-cg-transitives-Verb}.
\begin{figure}
\centerline{%
\begin{forest}
sn edges
...
(for more on unification see Section~\ref{sec-unifikation}). The unification of np:y with np: \relation{apples} results in np: \relation{apples} since \relation{apples}
is more specific than the variable y. Apart from its occurance in the term np:y, y occurs in the description of the verb in another position (s:
...
\end{figure}%
% Der semantische Beitrag von \emph{touch} ist $\lambda u\lambda v\relation{touch}(v,u)$. Der
% semantische Beitrag ($\alpha'$) wird durch den semantischen Teil der Regel in
% (\ref{Lexikonregel-Passiv-CG}) auf $\lambda y\lambda x \alpha'(x) (y)$ abgebildet. Setze man in
% diesen Ausdruck für $\alpha'$ die Bedeutung von \emph{touch} ein erhält man den Ausdruck in
% (\mex{1}a) und nach den entsprechenden Kombinations- und Reduktionsschritten einen Ausdruck, der der
% Bedeutung des Passivsatzes entspricht.
%\note{Bedeutung von touch muss andersrum sein -> Analyse geht  nicht auf}
% \eal
% \settowidth\jamwidth{(Kombination mit \relation{john})}
% \ex $\lambda y\lambda x \lambda u\lambda v~\relation{touch}(u,v)(x) (y)$ 
% \ex $\lambda y\lambda x \lambda u\lambda v~\relation{touch}(u,v)(x) (y)$ \relation{mary}      \jambox{(Kombination mit \relation{mary})}
% \ex $\lambda x \lambda u\lambda v~\relation{touch}(u,v)(x) (\relation{mary})$                 \jambox{($\beta$"=Reduktion)}
% \ex $\lambda x \lambda u\lambda v~\relation{touch}(u,v)(x) (\relation{mary})$ \relation{john} \jambox{(Kombination mit \relation{john})}
% \ex $\lambda u\lambda v~\relation{touch}(u,v)(\relation{john}) (\relation{mary})$             \jambox{($\beta$"=Reduktion)}
% \ex $\lambda v~\relation{touch}(\relation{mary},v)(\relation{john})$                          \jambox{($\beta$"=Reduktion)}
% \ex $\relation{touch}(\relation{mary},\relation{john})$                                       \jambox{($\beta$"=Reduktion)}
% \zl
% geht nicht auf
% \eal
% \settowidth\jamwidth{(Kombination mit \relation{john})}
% \ex $\lambda y\lambda x \lambda u\lambda v~\relation{touch}(v,u)(y) (x)$ 
% \ex $\lambda y\lambda x \lambda u\lambda v~\relation{touch}(v,u)(y) (x)$ \relation{mary}      \jambox{(Kombination mit \relation{mary})}
% \ex $\lambda x \lambda u\lambda v~\relation{touch}(v,u)(\relation{mary}) (x)$                 \jambox{($\beta$"=Reduktion)}
% \ex $\lambda x \lambda u\lambda v~\relation{touch}(v,u)(\relation{mary}) (x)$ \relation{john} \jambox{(Kombination mit \relation{john})}
% \ex $\lambda u\lambda v~\relation{touch}(v,u)(\relation{mary}) (\relation{john})$             \jambox{($\beta$"=Reduktion)}
% \ex $\lambda v~\relation{touch}(\relation{mary},v)(\relation{john})$                          \jambox{($\beta$"=Reduktion)}
% \ex $\relation{touch}(\relation{mary},\relation{john})$                                       \jambox{($\beta$"=Reduktion)}
% \zl
...
\end{tabular}
\z
\citet[Section~3.1]{Hoffmann95a-u} has proposed an analysis analagous to that of Japanese for Turkish\il{Turkish} and this could also be used in conjunction with an analysis
...
\mbox{}\citet[Section~1.2.4]{Steedman89a} proposes an analysis of long"=distance dependencies without movement or empty elements. For examples such as (\mex{1}), he assumes that
...
is a sentence missing an NP. We still require two further extensions of Categorial Grammar: type raising\is{type raising} and forward\is{composition!foward}
...
s/np. Figure~\vref{abb-CG-Komposition} shows how this works.\todostefan{textit sieht anders aus als
  mathmode, aber ohne textit werden die Munisse mit Leerzeichen gesetzt}
\begin{figure}
\centerline{%
\deriv{6}{
These\;apples  & Harry                & must & have & been & eating\\
...
%
%
np            & s/{(s\bs np)}        & {(s\bs np)}/vp & vp/\textit{vp-en} & \textit{vp-en}/\textit{vp-ing} & \textit{vp-ing}/np\\
              & \multicolumn{2}{c}{\forwardc}\\
              & \multicolumn{2}{c}{{{s}/vp}}\\
              & \multicolumn{3}{c}{\forwardc}\\
              & \multicolumn{3}{c}{{{s}/\textit{vp-en}}}\\
              & \multicolumn{4}{c}{\forwardc}\\
              & \multicolumn{4}{c}{{{s}/\textit{vp-ing}}}\\
...
%
%
st/(s/np)\;\;     & s/{(s\bs np)}        & {(s\bs np)}/vp & vp/\textit{vp-en} & \textit{vp-en}/\textit{vp-ing} & \textit{vp-ing}/np\\
              & \multicolumn{2}{c}{\forwardc}\\
              & \multicolumn{2}{c}{{{s}/vp}}\\
              & \multicolumn{3}{c}{\forwardc}\\
              & \multicolumn{3}{c}{{{s}/\textit{vp-en}}}\\
...
In additon to entries such as those in (\ref{le-relpron-Steedman}), we also need further entries to analyze sentences such as (\mex{1}), where
