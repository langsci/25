...
Some stellar works in the field of semantics making use of Categorial Grammar are those of Richard Montague \citeyearpar{Montague74a-u}.
...
The 2569"=page grammar of the \emph{Institut für Deutsche Sprache} in Mannheim \citep*{IDS97-not-crossreferenced} contains Categorial Grammar analyses in the relevant chapters.
...
The question as to how to analyze the pair of sentences in (\mex{1}) still remains unanswered.\footnote{%
	Thanks to Roland Schäfer\ia{Schäfer, Roland} (p.\,m., 2009) for pointing out these data to me.
}
\eal
...
\emph{gave} has the category ((s\bs np)/pp)/np, that is, the verb must first combine with an NP (\emph{the
  ...
  or assume additional rules for cases such as (\mex{0}b).\is{passive|)}\todostefan{NW:
    \citew{Dowty97a-u} head wrapping does this.}
...
in Figure~\ref{abb-CG-UDC-Ditrans} for (\mex{1}).
\ea
...
Kubota (p.c.\,2020) discussing this section and \citet{Kubota2020a} with me pointed out to me
...
\citet{Kubota2020a} compares Categorial Grammar with HPSG.
}
