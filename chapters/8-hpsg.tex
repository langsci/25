...
\ea
\onems[head-argument-phrase~]{
      phon  \phonliste{ das Buch gibt }\\
      synsem$|$loc$|$cat \ms{ head & \ibox{1}\\
                              subcat & \ibox{2} \sliste{ NP[\type{nom}], NP[\type{dat}] }
                            }\\
...
{ cat & \ms{ head & \ms[verb]
                     { vform & fin } \\
             subcat & \liste{ NP[\type{nom}]\ind{1}, NP[\type{dat}]\ind{2}, NP[\type{acc}]\ind{3}   } \\
           } \\
  cont &  \ms[mrs]
           { ind & \ibox{4} event \\
...
{ cat$|$subcat \liste{ []\ind{1}, []\ind{2}, []\ind{3}  } \\[1mm]
  cont  \ms[mrs]
           { ind & \ibox{4} event \\
             rels & \liste{ \ms[agent-goal-theme-rel~~]{ event & \ibox{4} \\
                                        agent & \ibox{1} \\
                                        goal  & \ibox{2} \\
                                        theme & \ibox{3} }}
           }
...
\ibox{3}.\footnote{
  ...
  corresponds to the unmarked order of arguments in the German clause. The version of the passive
  lexical rule that will be suggested below is compatible with both orders of arguments.
} The output
...
temporal adverbials (\mex{1}b), dative objects of psychological verbs (\mex{1}c) and objects in
passives (\mex{1}d) can be placed in the Vorfeld, even though
...
\glt `Does everybody know this man?'
\z
If one assumes that the initial element in the Mittelfeld is fronted, it is explained why the
