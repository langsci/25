...
(\citealp{ps,ps2}; see \citealp{FWPEvolution} for more on the history of HPSG). Like LFG, HPSG is part of so"=called West Coast linguistics. Another similarity to
LFG is that HPSG aims to provide a theory of competence which is compatible with performance
(\citealp{SW2011a,SW2015a,Wasow2020a}, see also Chapter~\ref{Abschnitt-Diskussion-Performanz}).  
...
and typed feature structures should consult Chapter~\ref{chap-feature-descriptions} first.
  ...
  grammar} (\citealp{King99a-u}; see also Chapter~\ref{chap-mts} of this book). 
The feature structures can be described with feature descriptions such as in (\mex{1}). Lexical entries, phrases
...
complexes in languages like German and Korean\il{Korean}. The Immediate Dominance\is{dominance!immediate}/Linear Precedence\is{linear precedence}\is{ID/LP grammar} format
...
Here, valence has been encoded twice. First, we have said something in the rules about what kind of elements can or must occur, and then we have stated
 ...
 entry of that head. There are the features \textsc{specifier} (\spr\isfeat{spr}) and
 \textsc{complements} (\comps\isfeat{comps}), whose values are lists containing descriptions of the
 elements that must combine with a head in order to yield a complete phrase. (\mex{1}) gives some examples for the verbs in (\ref{psg-valenz}):
\ea
\begin{tabular}[t]{@{}lll}
      verb             & \comps\\
...
\end{figure}%
%\addlines
In Figures~\ref{abb-peter-schlaeft} and~\ref{abb-Peter-Maria-erwartet}, one element of the \compsl is combined with its head in each
local tree. The elements that are combined with the selecting head are then no longer present in the \compsl of the mother node.
V[\comps \sliste{ }] corresponds to a complete phrase (VP or S). The boxes with numbers show the structure sharing (see Section~\ref{sec-strukturteilung}).
...
the \compsl is identical to another daughter in the tree.
...
or a complex noun phrase, it only matters that the linguistic object in question is complete (has an
...
ditransitive verb, as in Figure~\ref{er-das-buch-dem-mann-gibt-flat} on page~\pageref{er-das-buch-dem-mann-gibt-flat},
...
\end{schema}
Schema~\ref{schema-bin-prel} states the properties a linguistic object of the type \type{head"=complement"=phrase} must have.
...
[V{[\type{fin}, \comps \eliste]}, name=fin1
	...
	[V{[\type{fin}, \comps \sliste{ \ibox{1} }]}, name=fin2
		...
		[V{[\type{fin}, \comps \sliste{ \iboxsp{1}, \ibox{2} }]}, name=fin3
			[\ibox{3} NP{[\textit{acc}]}
				[das Buch;the book,roof]]
			[V{[\type{fin}, \comps \sliste{ \iboxsp{1}, \iboxsp{2}, \ibox{3} }]}, name=fin4
				[gibt;gives]]]]]	
	...
	[\ms{
		...
		[\ms{
			...
			[\ms{
                                                                                   ...
                                                                                   comps & \sliste{ \iboxsp{2}, \iboxsp{3}, \ibox{4} }
                                                                                    }
				[gibt;gives]]]]]	
\end{forest}
...
%
For more on theories of linking in HPSG, see \citew{Davis96a-u}, \citew{Wechsler91a-u} und
...
[V{[\comps \eliste]}
	...
	[V{[\comps \eliste]}
		...
		[V{[\comps \sliste{ \ibox{3} }]}
			...
			[{[V[\comps \sliste{ \iboxsp{3}, \ibox{4} }]}
				[\trace]]]]]]
\end{forest}
}
...
[V{[\comps \eliste]}
	...
	[\ibox{1} V{\feattab{
		...
		[V{\feattab{
			...
			[V{\feattab{
                              \textsc{dsl$|$cat$|$comps} \ibox{2},\\
                              \textsc{comps} \ibox{2} \sliste{ \iboxsp{3}, \ibox{4} } }}
				[\trace]]]]]
\end{forest}
...
[V\feattab{\textsc{comps} \eliste,\\ 
	...
	[V\feattab{\textsc{comps} \sliste{},\\
		...
		[V{[\comps \sliste{ \ibox{2} }]},  l sep=2\baselineskip
			[V{[\comps \sliste{ \iboxsp{3}, \ibox{4} }]},edge
...
in the entry for \emph{kennt} (\sliste{ \iboxsp{3}, \ibox{4} }) is present on the verb trace. The combination of the projection of the verb trace with
