...
As well as more general types, the type hierarchy of a particular language contains language"=specific types or those specific to a particular class
...
`give'. In Figure~\ref{Abbildung-Hierarchie}, there are different subtypes of \type{root}, the general type for roots: \type{intrans-verb} for intransitive
...
and unergative\is{verb!unergative} verbs and even strictly transitive verbs would have to be divided into further sub"=classes (see \citealp[Section~2]{Welke2009a}). 
...
derived from simple roots but still have to be inflected (\stem{lesbar} `readable', \stem{besing} `to extol'). Words are objects that do not inflect. Examples of these
...
(\emph{geliebt} `loved', \emph{besingt} `extols'). Relations between inflected words and (complex) stems can be formed again using derivation\is{derivation} rules. 
...
\citew[Section~3.2.7]{Mueller2002b}. All words can be combined to form phrases using dominance schemata. The hierarchy given here is of course by no means complete.
...
valid types and principles is innate since something being innate does not follow from the fact that it is present in all languages (also see Section~\ref{Abschnitt-Universalien-Zusammenfassung}).

\todostefan{Hier vielleicht noch etwas zur Variation sagen. Warum können benachbarte Dialekte nicht wild variieren?}
...
discussed the respective analyses of German in more detail in the Section~\ref{sec-case-asignment}
...
The interesting question is what will be the top"=most set if we consider enough languages. At
...
Persian nouns do not share all properties with English nouns.\footnote{
  ...
  \citew[Section 2]{Haspelmath2010a} for such a suggestion regarding case, e.\,g.\ Russian Dative,
...
\citet[\page 697]{Haspelmath2010b} writes: \emph{Generative linguists try to use as many crosslinguistic
...
for V2 languages but rule out languages with the verb in penultimate position (see \citew[\page
...
language external constraints: we simply do not have enough working memory to do such complex computations\is{performance}.

A variant of this argument comes from David Pesetzky\aimention{David Pesetzky} and was raised in Facebook discussions of an
article by Paul Ibbotson\aimention{Paul Ibbotson} and Michael Tomasello\aimention{Michael Tomasello} in the Guardian\footnote{
...
\gll Es kamen drei Männer zum Tor hinein.\\
     \expl{} came three man to.the door in\\
\glt `Three man came through the door.'
\z
...
genetic\is{gene} point of view and wait for other, probably functional, explanations of the Cinque data.
...
rather far"=reaching fundamental design decisions \citep{Fanselow2009a,SR2012a,Haider2014a}. It is interesting to note
...
unanalyzed data cannot refute grammatical hypotheses (see for instance \citealp[\page 454]{Freidin2009a}). In the very same way it has to be said that unanalyzed data
...
\citew[\page 481]{Goldberg2013b} for an explicit statement to this end) and in any case it should also be compatible with the Minimalist
world.
