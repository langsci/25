...
\spacebr{}the cake   to.eat.up   is.allowed.to he not\\
}
...
\gll [Den          Kuchen aufessen] darf          er nicht.\\
     \spacebr{}the cake   eat.up    is.allowed.to he not\\
...
not a bare infinitive, while \emph{darf} `is allowed to' takes a bare infinitive.
...
While we can reorder the parts of the VP and thereby derive (\ref{bsp-GPSG-anordnung}b), it is not
possible to place the subject at a lower position between the objects. Instead, a metarule can be
used to analyze sentences where the subject occurs between other arguments of the verb. This rule
relates phrase structure  rules to other phrase structure rules. A metarule can be understood as a
kind of instruction that creates another rule for each rule with a certain form and these newly
created rules will in turn license local trees. 

For the example at hand, we can formulate a metarule which says the following: if there is a rule
with the form ``V2 consists of something'' in the grammar, then there also has to be another rule
``V3 consists of whatever V2 consists $+$ an NP in the nominative''. In formal terms, this looks as
follows: 
\ea
\label{subjekt-meta}
V2  $\to$ W $\mapsto$\\
...
\z
W is a variable which stands for an arbitrary number of categories (W = \emph{what\-ever}). The
metarule creates the following rules in (\mex{1}) from the rules in (\ref{gpsg-regel-dat-ditransitiv}):
\ea
\begin{tabular}[t]{@{}l@{~$\to$~}l@{}}
...
\noindent
Now, the subject and other arguments both occur in the right"=hand side of the rule and can
therefore be freely ordered as long as no LP rules are violated.%
\is{metarule|)}
...
With the rule in (\mex{0}), it is possible that the type \sliste{ \type{e}, \sliste{ \type{e},
    \type{t} }} corresponds to an expression which still has to be combined with two expressions of
type \type{e} in order to result in an expression of \type{t}. The first combination step with
\type{e} will yield \sliste{ \type{e}, \type{t} } and the second step of combination with a further
\type{e} will give us \type{t}. This is similar to what we saw with $\lambda$"=expressions on 
...
\relation{like}(\relation{max}, \relation{lotte}), that is, an expression that is either true or false in the relevant world.
...
An N$'$ stands for a nominal expression of the kind  $\lambda$x child(x). This means if there is a specific individual which one can insert in place of the x, then we arrive at an
...
\gll [weil] der Mann dem Kind das Buch \emph{gestern} gab\\
	 \spacebr{}because the man the child the book yesterday gave\\
\glt `because the man gave the book to the child yesterday'
\ex 
\gll [weil] der Mann dem Kind \emph{gestern} das Buch gab\\
	 \spacebr{}because the man the child yesterday the book gave\\
\ex 
\gll [weil] der Mann \emph{gestern} dem Kind das Buch gab\\
	 \spacebr{}because the man yesterday the child the book gave\\
\ex 
\gll [weil] \emph{gestern} der Mann dem Kind das Buch gab\\
	 \spacebr{}because yesterday the man the child the book gave\\
\zl
...
\gll [weil] (oft) die Frau (oft) dem Mann (oft) hilft\\
     \spacebr{}because \spacebr{}often the woman \spacebr{}often the man \spacebr{}often helps\\
...
\gll [weil]    noch  gearbeitet wurde\\
     \spacebr{}because still worked     \AUX\\
...
\end{tabular}
\z
There is no NP on the right"=hand side of these rules which could be replaced by a \emph{von}-PP. If the passive
...
\glt `that the man gives the child the book'
\ex 
...
This section is for advanced readers. It compares GPSG with theories introduced later in the book,
...
was possible to explain the simultaneous extraction of elements from conjuncts (Across the Board Extraction\is{Across the Board Extraction}, \citealp[Section~4.2.4.1]{Ross67a,Williams78a}). The following examples from
