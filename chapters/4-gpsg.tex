...
sn edges
[{V2[\vform \type{inf}]}
  [N2 [den Kuchen;the cake,triangle] ]
...
from those which refer to \emph{linear precedence}\is{linear precedence} (LP): rules such as (\mex{1}) are to be understood as dominance rules, which do not
...
sn edges
[S
  [{N2[nom]} [der Mann;the man,triangle] ]
  [V2
    [{N2[dat]} [der Frau;the woman,triangle] ]
    [{N2[acc]} [das Buch;the book,triangle] ] 
    [V [gibt;gives] ] ] ]
\end{forest}}
...
\type{a} and \type{b} are types, then \sliste{ \type{a}, \type{b} } is also a type. Examples of complex types are \sliste{ \type{e}, \type{t} } and \sliste{ \type{e}, \sliste{
    \type{e}, \type{t} }}. We can define the following combinatorial rule for these kind of typed expressions:
\ea
...
With the rule in (\mex{0}), it is possible that the type \sliste{ \type{e}, \sliste{
	...
	page~\pageref{lambbbda-moegen}: $\lambda y \lambda x$ \relation{like}(x, y) has to combine with a y and an x. The result in this example was 
...
% Richter/Sterenfeld 2012: Es fehlt bei TYP(Det) = \sliste{ N$'$, NP } in den Klammern die TYP-Aufrufe
...
\citet*[\page 209]{GKPS85a} point out a redundancy in the semantic specification of grammars which follow the rule"=by"=rule
hypothesis\is{rule"=by"=rule hypothesis} (see Section~\ref{sec-PSG-Semantik}) since, instead of giving rule by rule instructions with regard to combinations, it suffices in many cases simply
...
Chapter~\ref{Abschnitt-Generative-Kapazitaet}) (\citealp{Thompson82a-u}; \citealp[\page 146]{Uszkoreit87a}). If one uses the Kleene star\is{Kleene star}\is{*},
...
%% possibility to have zero AdvPs leads to spurious ambiguities since both the original rule for V2 and
...
An alternative suggestion in a version of GPSG comes from \citep[\page 110]{Jacobs86a}: Jacobs' analysis is a rendering of the verb movement analysis in GB. He assumes that there
...
(\mex{1}).\footnote{%
  The \textsc{fin} feature has been omitted one some of the nodes since it is redundant: +\textsc{mc}"=verbs always require the \textsc{fin} value `+'.
}
\ea
...
[{V3[+\textsc{fin}, +\textsc{mc}]}
  [{N2[dat,+\textsc{top}]} [dem Mann;the man,triangle] ]
  [{V3[+\textsc{mc}]/N2[dat]}
...
page~\pageref{bsp-Fernabhaengigkeit} -- repeated here as (\mex{1}):
\eal
...
[{V3[+\textsc{fin},+\textsc{mc}]}
  ...
  [{V3[+\textsc{mc}]/N2[acc]}
    [{V[9,+\textsc{mc}]} [glaubt;believes] ]
    [{N2[nom]} [du;you] ] 
    [{V3[+dass,$-$\textsc{mc}]/N2[acc]} 
      [{}[dass;that] ]
      [{V3[$-$dass,$-$\textsc{mc}]/N2[acc]} 
         [{N2[nom]} [ich;I] ]
         [{V[6,$-$\textsc{mc}]} [gesehen habe;seen have,triangle] ] ] ] ] ]
\end{forest}
}
