...
\z
This problem can be solved using a feature that ensures that determines whether the left periphery of the \nbar is empty.
...
So, the simplest way to cope with the German data is the assumtion of an empty noun. Alternatively
...
In order to rule out the last two sentences, the grammar has to contain information about case. The folloinwg grammar will do the job:
\eal
...
This can then by inserted into the substitution node of \emph{treue} `loyal'. Similarly, the tree
