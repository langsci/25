...
\citew{Bierwisch92} and \citew{Vater2010a} for an historic overview.)
...
The variants of Chomskyan theories are often grouped under the heading \emph{Generative Grammar}\index{Generative Grammar}.
...
in the following way (also see \citealp[\page 162]{Chomsky95a-u}):
\begin{quote}
...
the form and potential power of transformations has to be restricted.\footnote{
	For more on the power of formal languages, see Chapter~\ref{Abschnitt-Generative-Kapazitaet}.
} 
...
An example for a parametrized principle is show in (\mex{1}):
\ea
...
D"=strucutre. For example, there are the following ordering variants for a sentence with a ditransitive verb in (\mex{1}):
\eal
\ex 
...
The subject and object are the arguments\is{argument} of the predicate. There are several terms used to describe the total selectional requirements such
...
a semantic role\is{semantic role} -- a so"=called theta role\is{theta role@$\theta$-role} -- and each role can only be assigned once \citep[\page 36]{Chomsky81a}: 
...
In\is{X theory@\xbar theory|(} GB, it is assumed that all syntactic structures licensed by the core grammar\is{core grammar}\footnote{
 \citet[\page 7--8]{Chomsky81a} distinguishes between a regular area of language which is determined by a grammar which
...
Lexical categories can be represented using binary features and a cross"=classification:\footnote{%
   ...
   \citew[Section~3.2]{Jackendoff77} for a cross"=classification of P, albeit with differing feature assignment.
} 
...
the \xbar template: \inull is a head, which takes the VP as its complements and thereby forms I$'$. The subject\is{subject} is the specifier\is{specifier} 
of the IP.
%
\begin{figure}
\centerline{%
...
empty and the auxiliary verb is moved to this position. Figure~\vref{Abb-GB-Ja-Nein} shows an analysis of (\mex{0}).
...
\emph{wh}-questions are formed by the additional movement of a constituent in front of the auxiliary. 
...
Until now, I have not yet discussed sentences without auxiliaries such as (\ref{ex-that-ann-reads-the-newspaper}). In order to analyze these kinds of sentences, one has to assume that the
...
like French\is{French} are SVO languages, whereas Welsh\il{Welsh} and Arabic\il{Arabic}
...
Before\is{passive} I turn to the analysis of the passive in Section~\ref{sec-case-asignment}, the first subsection will elaborate on the differences
...
\eal
\label{ex-passive-German-no-movment}
\ex 
...
sentence can be uttered is more restricted than (\mex{0}a) \citep{Hoehle82}. The order in (\mex{0}a)
...
behaviour of children does not change in an abrupt fashion as would be expected (\citealp[\page
...
\label{Asbchnitt-GB-Formalisierung}\label{sec-formalization-gb}
...
unaccusative verbs in the 70s. For German, see \citew{Grewendorf89a} and \citew{Fanselow92-ohne-anfuehrungszeichen}.
...
\citet[\page 5]{Stabler87a} use special purpose rules for auxiliary movement\is{auxiliary movement}.\footnote{
  ...
  are of a greater complexity (see Chapter~\ref{Abschnitt-Generative-Kapazitaet}) and grammars of corresponding complexity
...
from GPSG\indexgpsg) and some assumptions cannot be properly formalized and are simply ignored (see \citew{Briscoe97a} for details).
...
and LFG, see \citew{BK82a}. \citew{Kuhn2007a} offers a comparison of modern deriviational analyses with constraint"=based LFG\indexlfg and HPSG approaches.
